...
function FilterGallery(): React.ReactElement {
  ...
  const generateUniqueNormalizedOptions = (values: (string | undefined)[]): string[] => {
  ...
  const categoryOptions: CategoryOptions = useMemo(() => ({
    category: [],
    classification: generateUniqueNormalizedOptions(artMemo.map((art) => art.classification)),
    artist: generateUniqueNormalizedOptions(artMemo.map((art) => art.people?.[0]?.name)),
    century: generateUniqueNormalizedOptions(artMemo.map((art) => art.century)),
    medium: generateUniqueNormalizedOptions(artMemo.map((art) => art.medium)),
    culture: generateUniqueNormalizedOptions(artMemo.map((art) => art.culture)),
  }), [artMemo]);
